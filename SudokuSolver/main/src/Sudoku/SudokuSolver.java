...
public class SudokuSolver {
    ...
    // box-id --- box-member
    private final ArrayList<ArrayList<Integer>> BOXMEMBER;
    ...
    public SudokuSolver(SudokuBoard board) {
        ...
        // generate BOXMEMBER; box-id --- box-member
        this.BOXMEMBER = new ArrayList<>();
        for (int i = 0; i < 9; i++) {
            this.BOXMEMBER.add(new ArrayList<>());
        }
        for (int i = 0; i < this.BOXES.length; i++) {
            this.BOXMEMBER.get(this.BOXES[i]-1).add(i);
        }
    ...
    private int findMinCell() {
        ...
        // System.out.printf("emptyCellnum: %d\n", emptyCellNum);
        // solved
        if (emptyCellNum==0) {
            return -1;
        }
    ...
    // reduce the possibilities of the related (the same row/col/box) cells
    private void _reduce(int i) {
        ...
        // the same box
        for (int j:this.BOXMEMBER.get(this.BOXES[i]-1)) {
            if ((j != i) && (this.binaryBoard[j] > 0)) {
                _removeNum(j, reduceNum);
            }
        }
        // System.out.println(Arrays.toString(this.binaryBoard));
    }
    ...
    public void search() {
        ...
        while (true) {
            ...
            // System.out.printf("cell: %d; idx: %d\n", minCell, idx);
            // System.out.println(this.boardStack.size());
